...
func main() {
	...
	if dsn == "" {
		dsn = "postgres://postgres:postgres@localhost:5432/agrospray?sslmode=disable" // substitua pelos seus dados
	}
	...
	// Instanciando repositórios com conexão ao DB
	areaRepo := repositories.NewAreaRepo(db)
	fazendaRepo := repositories.NewFazendaRepo(db)
	loteRepo := repositories.NewLoteRepo(db)
	pulverizacaoRepo := repositories.NewPulverizacaoRepo(db)
	pulverizacaoAreaRepo := repositories.NewPulverizacaoAreaRepo(db)
	produtoRepo := repositories.NewProdutoRepo(db)
	...
	r.HandleFunc("/login", usuarioHandler.Login).Methods("POST")
	...
	log.Println("Servidor rodando em http://localhost:30000")
